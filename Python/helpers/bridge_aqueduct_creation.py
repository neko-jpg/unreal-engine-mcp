...
try:
    ...
    params_to_spec = None  # type: ignore[assignment]

logger = logging.getLogger("BridgeAqueductCreation")


def _spawn(
    sink, unreal, params: Dict[str, Any], tags: List[str] = None, all_actors: List[Dict[str, Any]] = None
) -> bool:
...
def build_suspension_bridge_structure(
    ...
    for i, tower_pos in enumerate(tower_positions):
        ...
        if _spawn(sink, unreal, base_params, tags=["bridge"], all_actors=all_actors):
            counts["towers"] += 1
        ...
        if _spawn(sink, unreal, main_shaft_params, tags=["bridge"], all_actors=all_actors):
            counts["towers"] += 1
        ...
        if _spawn(sink, unreal, top_params, tags=["bridge"], all_actors=all_actors):
            counts["towers"] += 1
        ...
        for side_offset in cable_offsets:
            ...
            if _spawn(sink, unreal, attachment_params, tags=["bridge"], all_actors=all_actors):
                counts["towers"] += 1
    ...
    for cable_idx, offset in enumerate(cable_offsets):
        for i in range(len(cable_points) - 1):
            ...
            if _spawn(sink, unreal, cable_params, tags=["bridge"], all_actors=all_actors):
    ...
    for i in range(deck_segments_x):
        for j in range(deck_segments_y):
            ...
            if _spawn(sink, unreal, deck_params, tags=["bridge"], all_actors=all_actors):
    ...
    for i in range(num_suspenders):
        ...
        for offset in cable_offsets:
            ...
            if _spawn(sink, unreal, suspender_params, tags=["bridge"], all_actors=all_actors):
                counts["suspenders"] += 1
...
def build_aqueduct_structure(
    ...
    for tier in range(tiers):
        ...
        for pier_idx in range(arches + 1):
            ...
            if _spawn(sink, unreal, pier_params, tags=["aqueduct"], all_actors=all_actors):
                counts["piers"] += 1
        ...
        for arch_idx in range(arches):
            ...
            for i in range(len(arch_points) - 1):
                ...
                if _spawn(sink, unreal, arch_params, tags=["aqueduct"], all_actors=all_actors):
    ...
    for i in range(deck_segments_length):
        for j in range(deck_segments_width):
            ...
            if _spawn(sink, unreal, deck_params, tags=["aqueduct"], all_actors=all_actors):
                counts["deck_segments"] += 1
    ...
    for side in [0, 1]:
        for i in range(deck_segments_length):
            ...
            if _spawn(sink, unreal, wall_params, tags=["aqueduct"], all_actors=all_actors):
