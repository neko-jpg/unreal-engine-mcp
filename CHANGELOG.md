...
- `Plugins/UnrealMCP/Source/UnrealMCP/UnrealMCP.Build.cs` — replaced inline Cesium / Niagara / Landscape / ControlRig probes with a single `AddOptionalModuleGates(Target)` machinery that emits `WITH_<KEY>_MCP=1/0` defines and conditional `Private/Editor` module deps for every optional UE 5.7 system needed by Waves 1–5 (Chaos, ChaosCloth, ChaosVehicles, Foliage, PCG, Water, MRQ, OnlineSubsystem, Iris, Localization editor, SourceControl, MetaSound, XR, OpenXR, AndroidPermission, Automation, DataValidation, GAS, StateTree, AnimRigging). Backwards-compatible: keeps emitting `WITH_CESIUM` and `WITH_LIVE_CODING` for existing code paths. (#70)
