...
pub async fn create_scene(
    ...
    let name = req.name.unwrap_or_else(|| req.scene_id.clone());
    let scene = repo.create_scene(&req.scene_id, req.description).await?;
    Ok(Json(success_response(serde_json::to_value(scene).unwrap_or_default())))
}
...
fn parse_transform(v: Option<serde_json::Value>) -> Transform {
    match v {
        Some(t) => {
            ...
            Transform {
                location: Vec3 {
                    x: loc.and_then(|l| l.get("x")).and_then(|v| v.as_f64()).unwrap_or(0.0),
                    y: loc.and_then(|l| l.get("y")).and_then(|v| v.as_f64()).unwrap_or(0.0),
                    z: loc.and_then(|l| l.get("z")).and_then(|v| v.as_f64()).unwrap_or(0.0),
                },
                rotation: Rotator {
                    pitch: rot.and_then(|r| r.get("pitch")).and_then(|v| v.as_f64()).unwrap_or(0.0),
                    yaw: rot.and_then(|r| r.get("yaw")).and_then(|v| v.as_f64()).unwrap_or(0.0),
                    roll: rot.and_then(|r| r.get("roll")).and_then(|v| v.as_f64()).unwrap_or(0.0),
                },
                scale: Vec3 {
                    x: scl.and_then(|s| s.get("x")).and_then(|v| v.as_f64()).unwrap_or(1.0),
                    y: scl.and_then(|s| s.get("y")).and_then(|v| v.as_f64()).unwrap_or(1.0),
                    z: scl.and_then(|s| s.get("z")).and_then(|v| v.as_f64()).unwrap_or(1.0),
                },
            }
        }
...
pub async fn upsert_object(
    ...
    if let Err(e) = validate_mcp_id(&req.mcp_id) {
        return Ok(Json(error_response("validation_error", &e)));
    }
    ...
    Ok(Json(success_response(serde_json::to_value(saved).unwrap_or_default())))
}
...
pub async fn list_objects(
    ...
    let repo = SurrealSceneRepository::new(state.db.clone());
    let objects = repo.list_desired_objects(&req.scene_id, req.include_deleted).await?;
...
pub async fn delete_object(
    ...
    Ok(Json(success_response(json!({ "tombstoned": true, "mcp_id": req.mcp_id }))))
}
...
pub async fn bulk_upsert_objects(
    ...
    for (i, obj_req) in req.objects.into_iter().enumerate() {
        ...
        let desired_name = obj_req.desired_name.unwrap_or_else(|| obj_req.mcp_id.clone());

        let mut obj = SceneObject {
            id: format!("scene_object:{}:{}", req.scene_id, obj_req.mcp_id),
            scene: format!("scene:{}", req.scene_id),
            group: obj_req.group_id.clone().or_else(|| req.group_id.clone()).map(|g| format!("scene_group:{g}")),
...
pub async fn plan_sync_route(
    ...
    let desired_objects = repo.list_desired_objects(&req.scene_id, true).await?;

    let unreal_client = UnrealClient::new(&state.config);
    let actual_actors = match unreal_client.get_actors_in_level().await {
        Ok(actors) => actors,
        Err(e) => {
            tracing::warn!("Could not reach Unreal for plan_sync: {e}. Proceeding with empty actual state.");
            Vec::new()
        }
    };
...
pub async fn apply_sync_route(
    ...
    let actual_actors = match unreal_client.get_actors_in_level().await {
        Ok(actors) => actors,
        Err(e) => {
            tracing::warn!("Could not reach Unreal for apply_sync: {e}. Proceeding with empty actual state.");
            Vec::new()
        }
    };
    ...
    Ok(Json(success_response(serde_json::to_value(result).unwrap_or_default())))
}
...
mod tests {
    ...
    fn object_or_empty_preserves_explicit_object() {
        assert_eq!(object_or_empty(Some(json!({ "path": "/Engine/BasicShapes/Cube.Cube" }))), json!({
