...
impl SurrealSceneRepository {
    ...
    pub async fn ensure_schema(&self) -> Result<(), AppError> {
        let queries = [
            ...
            // React-for-UE v3.0 operation ledger additions.
            "DEFINE FIELD OVERWRITE operation_id ON TABLE scene_operation TYPE option<string>;",
            "DEFINE FIELD OVERWRITE patch_id ON TABLE scene_operation TYPE option<string>;",
            "DEFINE FIELD OVERWRITE capability_id ON TABLE scene_operation TYPE option<string>;",
            "DEFINE FIELD OVERWRITE command ON TABLE scene_operation TYPE option<string>;",
            "DEFINE FIELD OVERWRITE target ON TABLE scene_operation FLEXIBLE TYPE option<object>;",
            ...
            // React-for-UE v3.0 desired-state additions (DEFINE FIELD OVERWRITE keeps schema migration compatible).
            "DEFINE FIELD OVERWRITE desired_hash ON TABLE scene_component TYPE string DEFAULT '';",
            "DEFINE FIELD OVERWRITE last_applied_hash ON TABLE scene_component TYPE option<string>;",
            "DEFINE FIELD OVERWRITE sync_status ON TABLE scene_component TYPE string DEFAULT 'pending';",
            "DEFINE FIELD OVERWRITE deleted ON TABLE scene_component TYPE bool DEFAULT false;",
            "DEFINE FIELD OVERWRITE revision ON TABLE scene_component TYPE int DEFAULT 1;",
            "DEFINE FIELD OVERWRITE last_operation_id ON TABLE scene_component TYPE option<string>;",
            "DEFINE FIELD OVERWRITE updated_by ON TABLE scene_component TYPE option<string>;",
            "DEFINE INDEX scene_component_scene_type ON TABLE scene_component COLUMNS scene, component_type;",
            "DEFINE INDEX scene_component_sync_status ON TABLE scene_component COLUMNS scene, sync_status;",
    ...
    pub async fn find_snapshot_by_name(
        &self,
        scene_id: &str,
        name: &str,
    ) -> Result<Option<(SceneSnapshot, Vec<SceneSnapshot>)>, AppError> {
        let mut snapshots = self.list_snapshots(scene_id).await?;
        snapshots.retain(|s| s.name == name);
        if snapshots.is_empty() {
            return Ok(None);
        }
        snapshots.sort_by(|a, b| b.created_at.cmp(&a.created_at));
        let chosen = snapshots[0].clone();
        Ok(Some((chosen, snapshots)))
    }
    ...
    /// Record an external operation generated by Python PatchExecutor.
    #[allow(clippy::too_many_arguments)]
    pub async fn record_external_operation(
        &self,
        scene_id: &str,
        ...
        self.db
            .query(
                "CREATE scene_operation SET                     scene = $scene,                     sync_run = $sync_run,                     operation_id = $operation_id,                     patch_id = $patch_id,                     capability_id = $capability_id,                     command = $command,                     mcp_id = $mcp_id,                     action = $action,                     reason = $reason,                     status = $status,                     target = $target,                     attempts = 1,                     created_at = time::now()"
            )
            .bind(("scene", format!("scene:{scene_id}")))
            .bind(("sync_run", "sync_run:external".to_string()))
            .bind(("operation_id", operation_id))
            .bind(("patch_id", patch_id))
            .bind(("capability_id", capability_id.clone()))
            .bind(("command", command.clone()))
            .bind(("mcp_id", target.get("entity_id").and_then(|v| v.as_str()).map(|s| s.to_string())))
            .bind(("action", command.or(capability_id).unwrap_or_else(|| "external".to_string())))
            .bind(("reason", reason.to_string()))
            .bind(("status", status.to_string()))
            .bind(("target", target))
            .await
            .map_err(|e| AppError::Database(format!("record external operation error: {e}")))?;
        Ok(())
    }
    ...
    pub async fn list_recent_operations(
        ...
        let mut operations: Vec<serde_json::Value> = self
            .db
            .query(
                "SELECT \
                    operation_id, patch_id, capability_id, command, mcp_id, action, reason, status, created_at \
                 FROM scene_operation WHERE scene = $scene ORDER BY created_at DESC LIMIT 100"
            )
            .bind(("scene", format!("scene:{scene_id}")))
            .await
            .map_err(|e| AppError::Database(format!("list recent operations error: {e}")))?
            .take(0)
            .map_err(|e| AppError::Database(format!("list recent operations parse error: {e}")))?;
    ...
    pub async fn upsert_component(
        ...
        // Prefer PatchCompiler-provided hashes when present so Python and Rust
        // share the same idempotency boundary. Fall back to Rust-side SHA1.
        let desired_hash = metadata
            .get("__sync")
            .and_then(|v| v.get("desired_hash"))
            .and_then(|v| v.as_str())
            .filter(|s| !s.is_empty())
            .map(|s| s.to_string())
            .unwrap_or_else(|| crate::sync::component_planner::compute_desired_hash(&properties));
        // operation_id may live inside metadata.__sync.operation_id
        let operation_id: Option<String> = metadata
            .get("__sync")
        ...
        let updated: Option<SceneComponent> = self
            ...
            .bind(("properties", properties))
            .bind(("metadata", metadata))
            .bind(("desired_hash", desired_hash))
            .bind(("operation_id", operation_id))
            .await
    ...
    /// Mark a component as synced after a successful apply.
    pub async fn mark_component_synced(
        &self,
        scene_id: &str,
        entity_id: &str,
        component_type: &str,
        ...
        self.db
            ...
            .await
            .map_err(|e| AppError::Database(format!("mark_component_synced error: {e}")))?;
        Ok(())
    }

    pub async fn mark_component_status(
        &self,
        scene_id: &str,
        entity_id: &str,
        component_type: &str,
        ...
        self.db
            .query(
                "UPDATE type::thing($table, $key) MERGE {                     sync_status: $sync_status,                     updated_at: time::now()                  }",
            )
            .bind(("table", "scene_component"))
            .bind(("key", record_key))
            .bind(("sync_status", sync_status.to_string()))
            .await
            .map_err(|e| AppError::Database(format!("mark_component_status error: {e}")))?;
        Ok(())
    }

    /// Backfill desired_hash for components that pre-date the v3.0 schema.
    pub async fn backfill_component_desired_hashes(&self) -> Result<usize, AppError> {
        let comps: Vec<SceneComponent> = self
            .db
            .query("SELECT * FROM scene_component WHERE desired_hash = '' OR desired_hash = NONE")
            .await
            .map_err(|e| AppError::Database(format!("backfill list error: {e}")))?
            .take(0)
            .map_err(|e| AppError::Database(format!("backfill parse error: {e}")))?;
        let mut updated = 0;
        for c in comps {
            ...
            self.db
                ...
                .bind(("hash", hash))
                .await
                .map_err(|e| AppError::Database(format!("backfill update error: {e}")))?;
            updated += 1;
        }
        Ok(updated)
    }
    ...
    pub async fn list_components(
        ...
        if sync_status.is_some() {
            query.push_str(" AND sync_status = $sync_status");
        }
        let mut q = self
            .db
            .query(query)
        ...
        if let Some(ss) = sync_status {
            q = q.bind(("sync_status", ss.to_string()));
        }
